 ...
 * Stroke All Test
 *
 * Comprehensive test for all stroke types, caps, joins, and modes using font paths
 * Font paths show corner behavior better than circles
 *
 * Grid Layout:
 * - Columns: overprint, cutout, join
 * - Rows: strokeCap × strokeJoin combinations (3×3=9 each) for each stroke type
 *   - solid: 9 combinations (rows 0-8)
 *   - dashed: 9 combinations (rows 9-17)
 *   - pattern: 9 combinations (rows 18-26)
 *   - dashed-pattern: 9 combinations (rows 27-35)
 *   Total: 36 rows
 *
 * Note: gradient stroke is tested separately in test-stroke-gradient.html
 */
...
const GRID_ROWS = 36  // solid×9 + dashed×9 + pattern×9 + dashed-pattern×9
...
const CANVAS_HEIGHT = CELL_HEIGHT * GRID_ROWS  // 8200
const FONT_SIZE = 280
...
// Generate all cap × join combinations for a stroke type
...
const STROKE_CONFIGS = [
  // Solid stroke - all cap × join combinations (9)
  ...generateCapJoinCombos('solid'),
  // Dashed stroke - all cap × join combinations (9)
  ...generateCapJoinCombos('dashed'),
  // Pattern stroke - all cap × join combinations (9)
  ...generateCapJoinCombos('pattern'),
  // Dashed-pattern stroke - all cap × join combinations (9)
  ...generateCapJoinCombos('dashed-pattern')
  // Note: gradient stroke is tested separately in test-stroke-gradient.html
]
...
window.draw = () => {
  ...
  // ====== MAIN GRID: 8 stroke configs × 3 modes ======
  drawMainGrid()
...
/**
 * Draw main grid (stroke configs × modes)
  ...
  switch (config.type) {
    case 'solid': {
      const stroke = {
        ...
        channelVals: [0, 100, 0],  // Green stroke
        strokeCap: config.cap || 'round',
        strokeJoin: config.join || 'miter'
      }
      return stroke
    }
