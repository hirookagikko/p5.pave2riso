 ...
 * ```
 */
export const applyEffectPipeline = (baseG, filter, halftone, dither, canvasSize, pipeline) => {
 ...
 * ```
 */
export const applyEffectPipelineWithOffset = (baseG, filter, halftone, dither, canvasSize, pipeline, baseX, baseY) => {
    // Calculate diagonal buffer settings
    const diag = calculateDiagonalBuffer(canvasSize, halftone, dither);
    // Apply filters first
    let processedG = applyFilters(baseG, filter);
 ...
 */
export const hasEffects = (filter, halftone, dither) => {
