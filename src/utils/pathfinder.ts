...
export const PathOffset = (
	...
	if (!paperInstance) {
		console.warn(
			"PathOffset: paper.js 0.12.4 is not loaded. Returning original path.",
		);
		return path;
	}
...
export const PathRemoveHoles = (path: PavePath): PavePath => {
	...
	const outerCurves: unknown[] = [];

	for (let i = 0; i < path.curves.length; i++) {
		const subPath = { curves: [path.curves[i]] } as PavePath;
	...
	return { curves: outerCurves } as PavePath;
