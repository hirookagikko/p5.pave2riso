...
export function createPaveUtils(deps) {
		...
		for (const path of paths) {
			...
			for (let i = 0; i < segments.length; i++) {
				...
				if (i === 0) {
					...
					const hasHandles =
						!prevSeg.handleOut.isZero() || !seg.handleIn.isZero();
			...
			if (path.closed && segments.length > 0 && vertices.length > 0) {
				...
				if (lastSeg && firstSeg && firstVertex) {
					const hasHandles =
						!lastSeg.handleOut.isZero() || !firstSeg.handleIn.isZero();
	...
	function removeHolesWithShoelace(path, emptyPath) {
		...
		for (const info of curveInfo) {
			...
			const sameSign =
				info.winding >= 0 === outerSign > 0 || info.winding === 0;
	...
	function PathSubtract(pathA, pathB) {
		...
		try {
			...
			if (
				...
				console.warn(
					"PathSubtract: Path subtraction failed. Returning empty path.",
				);
				return emptyPath;
			}
			throw e;
	...
	function PathExclude(pathA, pathB) {
		...
		try {
			...
			if (
				...
				console.warn(
					"PathExclude: Intersection covers entire area. Returning empty path.",
				);
				return emptyPath;
			}
			throw e;
	 ...
	 * @param {object} pathA - First path
	 * @param {object} pathB - Second path
	 * @returns {boolean} True if paths overlap, false otherwise
	 *
	 * @example
	 * const circle = Path.circle([100, 100], 50)
	 * const rect = Path.rect([120, 80], [60, 40])
	 * if (isPathsOverlap(circle, rect)) {
	 *   console.log('Paths overlap')
	 * }
	 */
	function isPathsOverlap(pathA, pathB) {
		...
		try {
			const bounds = Path.bounds(intersection);
			const width = bounds[1][0] - bounds[0][0];
			const height = bounds[1][1] - bounds[0][1];
			return width > 1e-10 && height > 1e-10;
		} catch (e) {
			console.warn("isPathsOverlap: Failed to get bounds", e);
			return false;
		}
	}
	...
	function PathOffset(path, distance, options) {
		...
		if (!paper) {
			console.warn(
				"PathOffset: paper.js is not loaded. Returning original path.",
			);
			return path;
		}
		if (!PaperOffset) {
			console.warn(
				"PathOffset: paperjs-offset is not loaded. Returning original path.",
			);
			return path;
		}
		...
		if (!ensurePaperInitialized()) {
			console.warn(
				"PathOffset: Failed to initialize Paper.js. Returning original path.",
			);
			return path;
		}
		...
		if (!paperPath) {
			console.warn(
				"PathOffset: Failed to convert path. Returning original path.",
			);
			return path;
		}
